...
If arguments were supplied, append them only to the second command. The script may read CodexBar's local auto-refreshed model-pricing catalog when present, but it must not invoke CodexBar. It may also make one unauthenticated read-only GET to the fixed `https://codex-resets.com/api/v1/status` endpoint. Do not read `auth.json`. If local session logs are unavailable, report the script error plainly.
